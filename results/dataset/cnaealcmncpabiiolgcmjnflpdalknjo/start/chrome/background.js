(function(e) {
	...
	if (!localStorage.getItem("newtab_url")) localStorage.setItem("newtab_url", chrome.extension.getURL("/start/index.html") + "?self");
	...
	chrome.runtime.setUninstallURL(user["firstRunLandingPage"] + "?ext_uninstall&id=" + chrome.runtime.id + "&ext=" + encodeURIComponent(chrome.i18n.getMessage("extName")));
	var a = utils.get;
	var o = utils.set;
	var l = false;
	var r = 0;
	...
	function i() {
		...
		utils.ajax_post(t, null, "json", function(e) {
			...
			if (e.webtab) {
				...
				if (localStorage.getItem("newtab_url") !== chrome.extension.getURL("/start/index.html") + "?self") localStorage.setItem("newtab_url", chrome.extension.getURL("/start/index.html") + "?self")
			}
			if (e.wl) {
	...
	var w = function(e, t) {
		...
		if (e == "click-Rate") {
			var o = "https://chrome.google.com/webstore/detail/" + chrome.runtime.id + "/reviews";
			chrome.tabs.create({
				url: o
			})
		...
		} else if (e == "click-Export") {
			chrome.tabs.create({
				url: "https://" + p + "/import-export-settings/?id=" + chrome.runtime.id + "&ext=" + encodeURIComponent(h())
			})
		} else if (e == "click-Feedback") {
			chrome.tabs.create({
				url: "https://" + p + "/feedback/?id=" + chrome.runtime.id + "&ext=" + encodeURIComponent(h())
			})
		} else if (e == "click-ShareFB") {
			chrome.tabs.create({
				url: "https://www.facebook.com/sharer/sharer.php?u=" + encodeURIComponent(a)
			})
		} else if (e == "click-ShareTW") {
			chrome.tabs.create({
				url: "https://www.twitter.com/share?url=" + encodeURIComponent(a)
			})
		} else if (e == "click-SharePI") {
			chrome.tabs.create({
				url: "https://pinterest.com/pin/create/button/?media=&description=&url=" + encodeURIComponent(a)
			})
		} else if (e == "click-ShareTU") {
			chrome.tabs.create({
				url: "https://www.tumblr.com/widgets/share/tool?canonicalUrl=" + encodeURIComponent(a)
			})
		} else if (e == "click-ShareVK") {
			chrome.tabs.create({
				url: "https://vk.com/share.php?url=" + encodeURIComponent(a)
			})
		} else if (e == "click-Donate") {
			var l = "https://" + p + "/donate/?id=" + chrome.runtime.id + "&ext=" + encodeURIComponent(h());
			chrome.tabs.create({
				url: l
			})
	...
	function O(t, a) {
		...
		try {
			...
			if (localStorage.getItem("favor_new_images") === "yes" && p !== Number(user["bg_img_list"])) {
				var b = JSON.parse(localStorage.getItem("mark_favor"));
				for (var I = p + 1; I <= Number(user["bg_img_list"]); I++) {
					if (b.indexOf(I + "") === -1) b.push(I + "")
				}
				localStorage.setItem("mark_favor", JSON.stringify(b));
		...
		setTimeout(function() {
			if (a === "onInstalled" || a === "onEnabled") {
				chrome.runtime.sendMessage(t.id, {
					...
					if (e.debug) console.log("sync " + chrome.runtime.id + " - " + t.id, chrome.runtime.lastError)
				})
			} else if (localStorage.getItem("wl")) {
				chrome.runtime.sendMessage(t.id, {
					set_wl: JSON.parse(localStorage.getItem("wl"))
				}, function(a) {
					if (e.debug) console.log("wl " + chrome.runtime.id + " - " + t.id, chrome.runtime.lastError)
				})
			}
		}, 300 + Math.floor(Math.random() * 700))
	}
	chrome.management.onInstalled.addListener(function(t) {
		if (e.debug) console.log("app-inst:", t);
		k(t, "onInstalled")
	});
	chrome.management.onEnabled.addListener(function(t) {
		if (e.debug) console.log("app-enable:", t);
		k(t, "onEnabled")
	});
		...
		if (!l) {
			chrome.management.get(a.id, function(e) {
				if (e.permissions && e.permissions.indexOf("newTabPageOverride") > -1 && e.permissions.indexOf("unlimitedStorage") > -1 && e.permissions.indexOf("topSites") > -1 && e.permissions.indexOf("management") > -1) {
					if (e.hostPermissions && (e.hostPermissions.indexOf("https://*.freeaddon.com/*") > -1 || e.hostPermissions.indexOf("https://*.sportifytab.com/*") > -1)) {
						return E(t, a, o)
					}
				}
			})
	...
	chrome.runtime.onMessage.addListener(function(t, a, o) {
		...
		if (typeof t === "string" && t.indexOf("click-") === 0) {
			...
			o({
				...
				updatePage: v,
				showMinor: l,
				bgImg: chrome.extension.getURL("/start/skin/images/" + s),
				debug: e.debug
			});
			return
		} else if (t.topSites) {
			...
			if (localStorage.getItem("newtab_url") !== chrome.extension.getURL("/start/index.html") + "?self") {
			...
			if (localStorage.getItem("random_all_newtab") == "yes") {
				...
				if (c.length > 0) {
					utils.getEnabledAppsInWhitelist(c, function(e) {
						...
						var o = "chrome-extension://" + t.id + "/start/index.html";
